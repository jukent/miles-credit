...
```{note}
This page documents the **Gen 1** loss configuration — the flat-key `loss` section
scored on the model's normalized output tensor. It remains fully supported and is
used by the Gen 1, LES, WRF, and downscaling trainers.

For **Gen 2**, see [Working with Loss Functions](Losses.md), which documents the
`{type, args}` section and `BaseLoss` — a per-variable loss computed on the
postblock output in physical units.

`load_loss` picks between the two by looking for a `type` key in the `loss`
section: present means Gen 2, absent means the flat keys described here.
```
...
If the built-in and existing custom loss functions don't perfectly fit your needs, you can define and register your own. Custom losses work the same way for Gen 1 and Gen 2 — you register the class, then reference it by key from the config.

```{note}
Earlier versions of this guide described editing a `base_losses.py` file to add an
entry to a hard-coded dictionary. That file no longer exists. Losses are now
resolved through the registry in `credit/losses/__init__.py`, and custom classes
are added with the `@register_loss` decorator plus a `custom_objects` entry — no
edit to the CREDIT source tree required. The steps below reflect the current API.
```

### 1. Write and register the loss class

Your custom loss should inherit from `torch.nn.Module` and implement a `forward` method that calculates the loss between the `target` and `prediction` tensors, in that argument order (CREDIT's convention). Decorate it with `@register_loss` to add it to the registry:
