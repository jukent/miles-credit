        ...
        for var in DSfirst.data_vars:
            if var in mean_ and var in std_:
                ...
                if "level" in DSfirst[var].dims:
                    mean_broadcast = (
                        mean_[var]
                        .expand_dims({"time": 1, "latitude": 192, "longitude": 288})
                    ...
                    DSfirst[var][:] = (DSfirst[var].values * std_broadcast.values) + mean_broadcast.values
...
def post_process(
    ...
    if avg_window:
        ...
        date_range = pd.date_range(
            start=f"{start_year}-{start_month:02d}", end=f"{end_year}-{end_month:02d}", freq="MS"
        )  # 'MS' is Month Start
