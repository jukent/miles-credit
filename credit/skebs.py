...
def concat_for_inplace_ops(y_orig, y_inplace_slice, ind_start, ind_end):
    ...
    """
    tensors = [y_orig[:, :ind_start], y_inplace_slice, y_orig[:, ind_end + 1 :]]
...
class BackscatterCNN(nn.Module):
    def __init__(self, in_channels, levels, nlat, nlon):
        """
...
class BackscatterUnet(nn.Module):
    def __init__(self, in_channels, levels, nlat, nlon, architecture, padding):
        """
...
class BackscatterPrescribed(nn.Module):
    def __init__(self, nlat, nlon, levels, std_path, sigma_max):
        """
        A prescribed array for each column for a uniform (across space) backscatter rate.
        the array weights are trainable. The array is initialized based on 1% of sigma_max * std_dev of wind
        """
        super().__init__()
        self.nlat = nlat
        self.nlon = nlon
...
class SKEBS(nn.Module):
    ...
    def __init__(self, post_conf):
        ...
        self.dissipation_scaling_coefficient = torch.tensor(
            post_conf["skebs"].get("dissipation_scaling_coefficient", 1.0)
        )
        self.dissipation_type = post_conf["skebs"]["dissipation_type"]
        if self.dissipation_type == "prescribed":
            self.backscatter_network = BackscatterPrescribed(
                self.nlat,
                self.nlon,
                self.levels,
            ...
            self.backscatter_network = BackscatterUnet(
    ...
    def initialize_skebs_parameters(self):
        ...
        self.register_buffer(
            "backscatter_filter",  # filter to zero out backscatter at top of model
            torch.cat(
                [
        ...
        def filter_init(max_wavenum, anneal_start):
            filter = torch.cat(
                [
                    ...
                    torch.linspace(1.0, 0.2, max_wavenum - anneal_start),
    ...
    def clip_parameters(self):
        ...
        self.alpha.data = self.alpha.data.clamp(self.eps, 1.0)
        self.variance.data = self.variance.clamp(self.eps, 10.0)
    ...
    def initialize_pattern(self, y_pred):
        ...
        self.multivariateNormal = MultivariateNormal(
            torch.zeros(2, device=y_pred.device), torch.eye(2, device=y_pred.device)
        )
    ...
    def forward(self, x):
        ...
        backscatter_pred = (
            self.dissipation_scaling_coefficient
        ...
        if (
            ...
            torch.save(
                backscatter_pred,
                join(self.debug_save_loc, f"backscatter_raw_{self.iteration}"),
            )
        ...
        if (
            ...
            torch.save(
                backscatter_pred,
                join(self.debug_save_loc, f"backscatter_{self.iteration}"),
            )
        ...
        u_chi, v_chi = self.getgrad(
            spec_coef
        )  # spec_coef represent vrt (non-divergent) coeffs so the perturbation will be non-divergent
