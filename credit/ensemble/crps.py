...
def calculate_crps_per_channel(ensemble_predictions: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    ...
    ensemble_predictions = ensemble_predictions.squeeze(1).squeeze(3)  # Shape: [ensemble_size, channels, height, width]
    ...
    for channel in range(num_channels):
        ...
        ensemble_predictions_flat = ensemble_predictions_channel.reshape(ensemble_size, -1).T
        # Shape: [height*width, ensemble_size]

        # Calculate CRPS for current channel
        channel_crps = crps_ensemble(observations=y_true_flat, forecasts=ensemble_predictions_flat)
