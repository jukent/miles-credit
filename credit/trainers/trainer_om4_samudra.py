...
class TrainerSamudra(BaseTrainer):
    ...
    def train_one_epoch(self, epoch, trainloader, optimizer, criterion, scaler, scheduler, metrics):
        ...
        if self.conf["data"].get("data_clamp", None) is None:
            flag_clamp = False
        else:
            flag_clamp = True
        ...
        post_conf = self.conf["model"].get("post_conf", {"activate": False})
        ...
        self.model.train()
        wet_mask = trainloader.dataset.wet[None, :, None, :, :].to(self.device)
        ...
        for _ in range(batches_per_epoch):
            ...
            while not stop_forecast:
                ...
                if forecast_step > 1:
                    ...
                    x_forcing_batch = batch["input"][:, -len(self.conf["data"]["dynamic_forcing_variables"]) :].to(
                        self.device
                    )
                ...
                if forecast_step in backprop_on_timestep:  # steps go from 1 to n
                    ...
                    with torch.autocast(enabled=amp, device_type="cuda"):
                        loss = criterion(y.to(y_pred.dtype) * wet_mask, y_pred).mean()
                ...
                # multi-timestep input
                    ...
                    # Determine which part of y_pred to concatenate
                    if "y_diag" in batch:
                        x = y_pred[:, :-varnum_diag, ...]  # drop diagnostic channels
                    else:
                        x = y_pred  # use all predicted channels
    ...
    def validate(self, epoch, valid_loader, criterion, metrics):
        ...
        # number of diagnostic variables
        varnum_diag = len(self.conf["data"]["diagnostic_variables"])
        input_length = self.conf["data"]["input_length"]
        output_length = self.conf["data"]["output_length"]
        ...
        wet_mask = valid_loader.dataset.wet[None, :, None, :, :].to(self.device)
            ...
            for _ in range(valid_batches_per_epoch):
                ...
                while not stop_forecast:
                    ...
                    if forecast_step == forecast_len:
                        ...
                        # ----------------------------------------------------------------------- #
                        # calculate rolling loss
                        loss = criterion(y.to(y_pred.dtype) * wet_mask, y_pred * wet_mask).mean()
                ...
                results_dict["valid_forecast_len"].append(forecast_len)
